...
% When you are using IEEE, you can add IEEEdoi if you want the DOI to be listed in your references
...
\begin{tikzpicture}[
    ...
    arrow/.style={->, thick},
    labelstyle/.style={midway, right, font=\scriptsize, draw=none}
]

\node (asset) {Top Level Asset \\ \scriptsize (Business)};
\node (field) [below=of asset] {Field \\ \scriptsize (Geographical)};
\node (reservoir) [below=of field] {Reservoir \\ \scriptsize (Subsurface)};
\node (sector) [below=of reservoir] {Sector \\ \scriptsize (Subsurface)};
\node (cluster) [below=of sector] {Cluster \\ \scriptsize (Operational)};
\node (wellpad) [below=of cluster] {Wellpad \\ \scriptsize (Operational)};
\node (well) [below=of wellpad] {Well \\ \scriptsize (Operational)};
\node (wellbore) [below=of well] {Wellbore \\ (Well String) \\ \scriptsize (Physical / Operational)};

% Arrows with relationship labels
\draw[arrow] (asset) -- node[labelstyle] {1:N} (field);
\draw[arrow] (field) -- node[labelstyle] {1:N} (reservoir);
\draw[arrow] (reservoir) -- node[labelstyle] {1:N} (sector);
\draw[arrow] (sector) -- node[labelstyle] {1:N} (cluster);
\draw[arrow] (cluster) -- node[labelstyle] {1:N} (wellpad);
\draw[arrow] (wellpad) -- node[labelstyle] {1:N} (well);
\draw[arrow] (well) -- node[labelstyle] {1:1} (wellbore);

\end{tikzpicture}
\caption{Oil and gas field asset hierarchy}
\label{fig:asset_hierarchy}
\end{figure}

\section{Oil and Gas Wells}
Hydrocarbon production wells are broadly classified into oil wells and gas wells based on the dominant produced phase. In practice, both types involve multiphase flow consisting of oil, gas, and water, which introduces significant complexity in modeling and measurement. Accurate estimation of phase flow rates is essential for reservoir management, production optimization, and allocation \citep{falcone2009multiphase, kolev2016multiphase}.

\subsection{Oil Wells}
Oil wells primarily produce liquid hydrocarbons, often accompanied by associated gas and water. Their behavior is typically governed by multiphase liquid-dominated flow.

\subsubsection{Characteristics}
\begin{itemize}
\item Oil is the dominant phase, with gas present as associated or dissolved gas.
\item Water production increases over time due to reservoir depletion and water breakthrough.
\item Flow is multiphase and generally liquid-dominated.
\item Artificial lift methods such as gas lift or \abbr{Electric Submersible Pumps}{ESP} are often required.
\end{itemize}

\subsubsection{Flow Behavior}
\begin{itemize}
\item Flow is governed by pressure drawdown and inflow performance relationships.
\item Multiphase interactions between oil, gas, and water lead to complex flow regimes \citep{brill1999multiphase}.
\item Increasing water cut impacts production efficiency and measurement accuracy \citep{falcone2009multiphase}.
\end{itemize}

\subsubsection{Measurement Challenges}
\begin{itemize}
\item Accurate phase separation is difficult, particularly at high water cut.
\item \abbr{Multiphase Flow Meters}{MPFM} exhibit varying accuracy depending on flow regime \citep{falcone2009multiphase}.
\item Well test data is reliable but sparse, whereas continuous sensor data is indirect and requires modeling.
\end{itemize}

\subsection{Gas Wells}
Gas wells primarily produce natural gas, with relatively small amounts of condensate and water. Their behavior is typically gas-dominated and strongly influenced by pressure conditions.

\subsubsection{Characteristics}
\begin{itemize}
\item Gas is the dominant phase, often produced at high velocities.
\item Liquid phases are typically minor but can significantly affect flow behavior.
\item Flow rates are highly sensitive to pressure differences between reservoir and wellbore.
\end{itemize}

\subsubsection{Flow Behavior}
\begin{itemize}
\item Gas flow is commonly modeled as a pressure-driven process.
\item Strong dependence on reservoir pressure and surface conditions.
\item Liquid loading can occur when gas velocity is insufficient to transport liquids, reducing production efficiency \citep{brill1999multiphase}.
\end{itemize}

\subsubsection{Measurement Challenges}
\begin{itemize}
\item Gas flow measurements are sensitive to pressure and temperature variations.
\item Small amounts of liquid can significantly impact measurement accuracy.
\item Indirect measurement techniques such as virtual flow metering are often required for continuous estimation.
\end{itemize}

\section{Producer and Injector Wells}
\begin{itemize}
    \item \textbf{Production Wells}: Production wells are the primary pathways through which hydrocarbons - oil, gas, and associated formation water, are extracted from a reservoir and transported to surface processing facilities. Their core function is to withdraw fluids from subsurface formations in a controlled manner that optimizes recovery while maintaining well integrity and reservoir performance. The performance of a production well is driven by the pressure differential between the reservoir and the wellbore \citep{ahmed2010reservoir}. Early in a field's life, natural energy such as solution gas drive, water drive, gas cap expansion, or aquifer support pushes fluids toward the wellbore. As reservoir pressure declines over time, production wells may transition to artificial lift systems such as gas lift, ESP, \abbr{Progressive Cavity Pumps}{PCP}, or \abbr{Rod Pumps}{RP} to sustain flow rates and overcome the hydrostatic and frictional losses in the wellbore \citep{brown1984artificial}. The choice of artificial lift depends on fluid properties, well depth, deviation, GOR, water cut, and economic considerations.
    \item \textbf{Injection Wells}: Injection wells are specialized wellbores designed to introduce fluids - such as water, gas, or chemical mixtures into subsurface formations for the purposes of reservoir pressure maintenance, \abbr{Enhanced Oil Recovery}{EOR} \citep{lake2014enhanced}, or safe disposal of produced water. Unlike production wells, which are engineered to withdraw hydrocarbons, injection wells primarily function as input conduits, pushing fluids into the reservoir to support long-term field productivity. Their role is essential in secondary and tertiary recovery processes, as they improve sweep efficiency and maintain reservoir pressure, thereby enhancing the deliverability of adjacent production wells.
\end{itemize}

\section{Production variables and ratios}
Accurate characterization of multiphase production is fundamental to oil well performance analysis, reservoir management, and virtual flow metering (VFM). Production from oil wells typically consists of three primary phases: oil, water, and gas, each contributing to the overall flow behavior in the wellbore and surface facilities.
To describe and quantify this multiphase production, a set of key variables and derived ratios are commonly used. These include phase flow rates such as oil rate ($q_o$), water rate ($q_w$), gas rate ($q_g$), and total liquid rate ($q_L$), as well as diagnostic ratios such as water cut (WC), gas--oil ratio (GOR), and water--gas ratio (WGR).
Table~\ref{tab:production-variables} lists the common production variables and ratios used.

\begin{table}[!htb]
\uomTable{Production Variables and Ratios}{tab:production-variables}{
\begin{tabularx}{0.9\textwidth}{p{3cm} X p{1.5cm} p{1.5cm}} 
\hline
\textbf{Variable} & \textbf{Description} & \textbf{SI Unit} & \textbf{Field Unit} \\
\hline
Oil Rate ($q_o$) & Volumetric flow rate of liquid hydrocarbons produced from the well. & Sm$^3$/day & STB/day \\
Water Rate ($q_w$) & Volumetric flow rate of produced water. & Sm$^3$/day & STB/day \\
Gas Rate ($q_g$) & Volumetric flow rate of produced gas at standard conditions. & Sm$^3$/day & SCF/day \\
Liquid Rate ($q_L$) & $q_L = q_o + q_w$, governed by inflow performance and pressure drawdown. & Sm$^3$/day & STB/day \\
Water Cut ($wc$) & $wc = \dfrac{q_w}{q_L} = \dfrac{q_w}{q_o + q_w}$, representing the fraction of water in the liquid stream. & -- & -- \\
Gas Oil Ratio (GOR) & $GOR = \dfrac{q_g}{q_o}$ & Sm$^3$/Sm$^3$ & SCF/STB \\
Water Oil Ratio (WOR) & $WOR = \dfrac{q_w}{q_o}$ & Sm$^3$/Sm$^3$ & bbl/bbl \\
Water Gas Ratio (WGR) & $WGR = \dfrac{q_w}{q_g}$, useful in gas-dominated or gas-lifted systems. & Sm$^3$/Sm$^3$ & bbl/MMscf \\
Gas Volume Fraction (GVF) & $GVF = \dfrac{q_g}{q_g + q_L}$, indicating the proportion of gas in the total produced stream. & -- & -- \\
\hline
\end{tabularx}}{}
\end{table}


\section{Well Instrumentation}
The configuration of instrumentation in an oil well varies depending on the operational objectives and reservoir characteristics. In general, sensors are strategically installed along the wellbore and surface facilities to monitor drilling, production, and safety-related parameters. The wellbore, often referred to as the borehole, is the cylindrical hole drilled from the surface into subsurface formations during the development of oil, gas, water, or geothermal wells. Figure~\ref{fig:well-instrumentation} illustrates the typical placement of instrumentation across different locations of a producing well.
...
% Ground level (aligned with wellhead)
\draw[dashed] (-3,0) -- (0,0);
\node[left] at (-3,0) {Ground Level};

% Wellhead (at ground level)
\draw[thick] (0,0) -- (0,0.3);

% Surface sensors (clean separation)
\node[right] at (-3.0,0.8) {Wellhead (whp / wht)};

% Flowline (on ground level)
\draw[thick] (0,0) -- (2,0);
...
\draw (2.4,0) -- (2.4,0.2);
\draw (2.4,0) -- (2.4,-0.2);
\draw (2.0,-0.2) -- (2.4,0.2);
\draw (2.0,0.2) -- (2.4,-0.2);
\node[below] at (2.2,-0.5) {dcr (choke)};

% Downstream pressure
\node[above] at (2.2,0.4) {dcp};

% Production arrow
\draw[arrow] (2.4,0) -- (4.5,0);
\node[right] at (4.5,0) {To Separator};

% Wellbore (starts from ground level)
\draw[thick] (0,0) .. controls (-0.3,-1.2) and (-1.5,-2.5) .. (-3.0,-6.4);
\node at (-0.2,-3.0) {Wellbore};

% Reservoir
\draw[thick] (-3.2,-8.3) ellipse (4.2cm and 2.1cm);
\node at (-3.2,-8.3) {Reservoir};

% Downhole sensors (spaced properly)
\node[right] at (-2.6,-6.0) {Downhole (dhp / dht)};

\draw (0,0) circle (0.08);
\draw (-3.0,-6.4) circle (0.08);

\end{tikzpicture}
\caption{Well instrumentation setup}
\label{fig:well-instrumentation}
\end{figure}

\begin{table}[!htb]
\uomTable{Well  Instrumentation}{tab:well-instrumentation}{
\begin{tabularx}{0.9\textwidth}{p{1cm} X p{1.5cm} p{1.5cm}} 
\hline
\textbf{Sensor} & \textbf{Description} & \textbf{Unit} & \textbf{Field Unit} \\
\hline
dhp & Downhole pressure & bara & psi \\
dht & Downhole temperature & \textdegree C & \textdegree F \\
whp & Wellhead pressure & bara & psi \\
wht & Wellhead temperature & \textdegree C & \textdegree F \\
dcp & Downstream choke pressure & bara & psi \\
...
\begin{enumerate}
    ...
    \item The research considers \textit{sparse and irregular ground-truth flow measurements}, such as those obtained from periodic well tests, reflecting realistic industrial data availability constraints.
...
Physics-based VFM approaches rely on first-principles models derived from reservoir engineering, fluid mechanics, and thermodynamics. Classical petroleum engineering formulations describe the relationship between reservoir pressure, flowing bottom-hole pressure, and production rate using inflow performance relationships (IPRs) \citep{dake1978fundamentals}. These formulations are widely used in nodal analysis to predict well performance under steady-state conditions.
...
\begin{tikzpicture}[
    ...
    every node/.style={draw, rounded corners, align=center, font=\small, minimum width=4.2cm, minimum height=2.2cm},
    arrow/.style={->, thick}
]

% Top row
\node (data) { \textbf{Well Data} \\ 
    $\bullet$ Operational data \\ 
    $\bullet$ Geometrical data \\ 
    $\bullet$ Sparse well test data };

\node (pre) [right=of data] { \textbf{Data Preprocessor} \\ 
    $\bullet$ Unit conversions \\ 
    $\bullet$ Feature enrichment \\ 
    $\bullet$ Data sanitization };

\node (phys) [right=of pre] { \textbf{Physics-Based Inflow Model (Per Well)} \\ 
    $\bullet$ Liquid inflow \\ 
    $\bullet$ Logistic water-cut \\ 
    $\bullet$ Gas rate equation };

% Middle row
\node (feat) [below=of phys] { \textbf{Feature Construction for Residual ML} \\ 
    $\bullet$ Measured variables \\ 
    $\bullet$ Lagged variables \\ 
    $\bullet$ Physics outputs ($q_o^{phys}, q_w^{phys}, q_g^{phys}$) };

\node (ml) [left=of feat] { \textbf{ML Residual Learning Model (Global)} \\ 
    $\bullet$ Oil residuals \\ 
    $\bullet$ Gas residuals \\ 
    $\bullet$ WGR residuals \\ 
    $\bullet$ Regime-aware };

\node (bias) [left=of ml] { \textbf{Per-Well Bias Calibration} \\ 
    $\bullet$ Mean log-space residual \\ 
    $\bullet$ Separate bias (oil, gas, WGR) \\ 
    $\bullet$ Applied before constraints \\ 
    $\bullet$ No bias for unseen wells };

% Bottom row
\node (hyb) [below=of ml] { \textbf{Hybrid Combination \& Gating Logic} \\ 
    $\bullet$ Oil: always corrected \\ 
    $\bullet$ Water: gated by physics \\ 
    $\bullet$ Gas: always corrected \\ 
    $\bullet$ Residual clipping \\ 
    $\bullet$ Physical constraints };

\node (out) [right=of hyb] { \textbf{Hybrid Physics--ML Output} \\ 
    $\bullet$ Final rates ($q_o, q_w, q_g$) \\ 
    $\bullet$ Bias + constraint enforced };

% Arrows
\draw[arrow] (data) -- (pre);
\draw[arrow] (pre) -- (phys);
\draw[arrow] (phys) -- (feat);
\draw[arrow] (feat) -- (ml);
\draw[arrow] (ml) -- (bias);
\draw[arrow] (bias) -- (hyb);
\draw[arrow] (hyb) -- (out);
...
\caption{Hybrid physics-informed residual learning architecture for virtual flow metering}
...
\subsection{Residual Learning using Machine Learning}
...
The proposed framework is designed to support scalable deployment across multiple wells by combining well-specific physics modeling with a globally trained residual learning component. For each well, the physics-based model is calibrated independently to capture well-specific characteristics such as reservoir pressure, inflow behavior, and operational conditions. In contrast, the machine learning residual model is trained across data from all wells, enabling it to learn shared patterns in the discrepancies between physics-based predictions and observed well-test measurements.

This hybrid structure allows the model to retain physically interpretable behavior at the well level while leveraging cross-well information to improve predictive accuracy. As demonstrated in both temporal and spatial generalization experiments, the residual model captures transferable correction patterns that generalize effectively to unseen wells under the \abbr{Leave-One-Well-Out}{LOWO} evaluation framework.

For deployment, the framework supports prediction based on available input measurements such as pressure, temperature, and choke settings. 
To facilitate practical implementation, the framework includes model persistence capabilities that allow calibrated physics parameters and trained residual models to be stored and reused. This enables efficient deployment in operational environments, where models can be initialized using historical data and applied to new wells with minimal additional calibration.
...
The dataset used in this research consists of multi-source production and operational measurements collected from a set of producing gas wells. The data integrate well test measurements, MPFM data, and permanently installed surface and downhole sensor readings. 
This study uses data from the same offshore gas field as \citep{nemoto2023cloud}, although the wells are not identical, still allowing for a meaningful comparison under similar field conditions.
...
The dependent variables considered in this study are the phase flow rates - oil rate ($q_o$), water rate ($q_w$), and gas rate ($q_g$) - primarily obtained from well test measurements. 
...
\begin{table}[!htb]
\uomTable{Gas Lift Variables}{tab:gaslift_vars}{\begin{tabularx}{0.9\textwidth}{lYYYY} 
\hline
Variable & Description & Unit \\
\hline
gl\_mass\_rate & Gas lift injection mass rate & kg/h \\
gl\_open\_ratio & Fraction of active gas lift valves & unitless (ratio) \\
\hline
\end{tabularx}}{}
\end{table}
...
\begin{itemize}
    ...
    \item MPFM measurements, while continuous, can exhibit \textbf{bias, drift, salinity sensitivity},
          ...
          MPFM estimations are instead used as an industry-standard reference to assess the performance of the proposed model.
...
oil, water, and gas flow rates using measurable wellhead and downhole variables \citep{ahmed2010reservoir}.
...
In practical production environments, detailed mechanistic multiphase flow models require extensive fluid property data, well geometry, and calibration effort, which may not be consistently available for all wells \citep{beggs1973study, ahmed2010reservoir}. Moreover, such models may still suffer from uncertainty under changing operating conditions. Therefore, a simplified physics-based formulation is adopted, prioritizing robustness, interpretability, and compatibility with sparse supervision.
...
\begin{itemize}
    \item The total liquid rate \(q_L(t)\) follows a quadratic inflow relationship
        with respect to the normalized flowing downhole pressure \(P_{\mathrm{wf}}/P_{\mathrm{res}}\) \citep{vogel1968inflow}.
...
The total liquid rate is expressed as \citep{vogel1968inflow}:
...
Phase splitting between oil and water is modeled using a logistic water-cut closure. Logistic functions are widely used in petroleum engineering to represent bounded variables such as water cut due to their smoothness and asymptotic behavior \citep{hastie2009elements}.

Water cut is defined as:

\begin{equation}
wc = wc_{min} + (wc_{max} - wc_{min}) \cdot \sigma\left(\mathbf{X}_{wc} \mathbf{A}\right),
\end{equation}

where:
\begin{itemize}
    \item $wc$ is the water cut,
    \item $\sigma(\cdot)$ denotes the logistic (sigmoid) function,
    \item $\mathbf{X}_{wc}$ is a feature vector constructed from operational variables such as choke opening, pressures, temperatures, and interaction terms,
    \item $\mathbf{A}$ is a vector of learnable coefficients,
    \item $wc_{min}$ and $wc_{max}$ are lower and upper bounds on water cut.
\end{itemize}

In this work, $wc_{min} = 0.02$ and $wc_{max} = 0.98$, ensuring that predicted water cut remains within physically plausible limits while avoiding numerical instability near the extremes.

The logistic function can be written explicitly as:

\begin{equation}
\sigma(z) = \frac{1}{1 + e^{-z}}
\end{equation}

Substituting the definition of the logistic function, the water-cut model can be written explicitly as:

\begin{equation}
wc = wc_{min} + (wc_{max} - wc_{min}) \cdot 
\frac{1}{1 + \exp\left(-\sum_{i=1}^{n} X_{wc,i} A_i\right)}
\end{equation}

The logistic formulation provides a smooth and nonlinear mapping between operational conditions and phase behavior, enabling the model to capture transitions between oil-dominated and water-dominated flow regimes. The inclusion of operational variables allows the model to account for the influence of pressure drawdown, thermal conditions, and choke regulation on phase distribution.
...
which enforces consistency with the total liquid rate:

\begin{equation}
q_L = q_o + q_w.
\end{equation}

The parameters $\mathbf{A}$ are estimated during model calibration as part of the overall nonlinear least squares optimization using well-test measurements of oil and water rates. Consequently, water cut is not directly fitted but is implicitly inferred through the coupled phase-splitting relationships.

\subsection{Gas Flow Modelling}
\label{subsec:gas_rate}
Gas production is modeled using a pressure-driven formulation modulated by choke effectiveness. In producing wells, gas flow is influenced by reservoir pressure drawdown and surface flow restrictions imposed by choke settings. Classical gas deliverability theory indicates that gas flow depends on pressure-squared differences between the reservoir and the wellbore \citep{ahmed2010reservoir, rawlins1935backpressure}. 
Classical gas deliverability theory is often expressed as:

\begin{equation}
q_g = C \left(P_{res}^2 - P_{wf}^2\right)^n,
\end{equation}

% where $C$ is a productivity constant and $n$ is a flow exponent reflecting the flow regime \citep{rawlins1935backpressure}. This formulation is based on compressible gas flow theory and is widely used in gas well performance analysis.

In contrast, the present work adopts a simplified pressure-drawdown formulation using a square-root dependency. This approximation provides improved numerical stability and computational efficiency while retaining the primary dependence on pressure drawdown. The gas flow rate is expressed as:

\begin{equation}
q_{g,res} = C_g \cdot S_g \cdot \sqrt{P_{res} - P_{wf}} \cdot \sigma\!\left(k_{ch}(ch - ch_0)\right),
\end{equation}

where:
\begin{itemize}
    \item $q_{g,res}$ is the reservoir gas production rate,
    \item $C_g$ is a gas flow coefficient,
    \item $S_g$ is a scaling factor accounting for gas properties,
    \item $P_{res}$ and $P_{wf}$ are reservoir and bottom-hole pressures, respectively,
    \item $ch$ denotes the choke opening,
    \item $k_{ch}$ and $ch_0$ control choke sensitivity and activation threshold,
    \item $\sigma(\cdot)$ denotes the logistic (sigmoid) function.
\end{itemize}

The square-root pressure dependency provides a simplified approximation of pressure-driven gas flow, offering numerical stability and robustness across varying operating conditions. The logistic choke term captures the nonlinear influence of choke opening on gas throughput while ensuring smooth and bounded behavior, thereby avoiding discontinuities associated with piecewise choke models.

When gas lift is present, an additional contribution is included:

\begin{equation}
q_g = q_{g,res} + q_{g,lift},
\end{equation}

\begin{equation}
q_{g,lift} = C_{gl} \cdot \dot{m}_{gas} \cdot OR,
\end{equation}

where $C_{gl}$ is a gas lift efficiency coefficient, $\dot{m}_{gas}$ is the injected gas mass rate, and $OR$ is the gas lift opening ratio. In the absence of gas lift signals, this term is set to zero.

This formulation provides a computationally efficient and stable approximation of gas flow behavior, while retaining the key dependency on pressure drawdown and operational controls.
...
The initial parameters estimates used for the calibration are summarized in table~\ref{tab:physics_calibrated_parameters}.
...
The parameter estimation problem is posed as a constrained nonlinear least squares optimization \citep{levenberg1944method, nocedal2006numerical}:
\begin{equation}
...
These bounds enforce physical plausibility, such as non-negative productivity indices, reasonable reservoir pressures, and monotonic choke response, and improve numerical stability and convergence behavior.

The optimization is solved using a trust-region-based nonlinear least squares algorithm, which iteratively linearizes the residual function around the current parameter estimate and updates the parameters by solving a sequence of local quadratic subproblems \citep{nocedal2006numerical}. This approach is well-suited for moderately nonlinear models and provides robust convergence under noisy and sparse data conditions.
...
Following optimization, calibration quality is assessed using residual diagnostics and goodness-of-fit metrics computed on the measured data.
...
\subsubsection{Numerical Example: Physics-Based Calibration for Well}
\subsubsection*{Step 0: Example Data}
\begin{table}[!htb]
\uomTable{Well test data (liquid approximated as $q_L = q_o + q_w$)}{tab:sample_well_test_data}{
\begin{tabularx}{\textwidth}{YYYYYY}
\hline
$P_{wf}$ (bar) & Choke & $q_o^{\text{WT}}$ & $q_w^{\text{WT}}$ & $q_g^{\text{WT}}$ & $q_L^{\text{WT}}$ \\
\hline
140.80 & 0.528 & 106.56 & 0.01 & 12956.36 & 106.57 \\
145.24 & 0.444 & 82.95  & 0.02 & 9954.12  & 82.97 \\
132.61 & 0.817 & 156.55 & 0.05 & 19641.97 & 156.60 \\
132.58 & 0.814 & 156.54 & 0.04 & 19736.54 & 156.58 \\
131.86 & 0.840 & 160.34 & 0.07 & 20500.92 & 160.41 \\
\hline
\end{tabularx}}{}
\end{table}


\subsubsection*{Step 1: Estimate Reservoir Pressure}
Given:
\[
P_{wf,\max} = 145.24 \text{ bar}, \quad h = 1800.787 \text{ m}, \quad \rho = 850 \text{ kg/m}^3
\]

Hydrostatic pressure:
\[
P_h = \frac{\rho g h}{10^5}
= \frac{850 \times 9.81 \times 1800.787}{10^5}
\approx 150.20 \text{ bar}
\]

Reservoir pressure estimate:
\[
P_{\text{res}} = P_{wf,\max} + P_h = 145.24 + 150.20 = 295.44 \text{ bar}
\]

Bounds:
\[
P_{\text{res}}^{\text{lower}} \approx 145.24 + 0.7 \times 150.20 = 250.38
\]
\[
P_{\text{res}}^{\text{upper}} \approx 145.24 + 1.3 \times 150.20 = 340.50
\]


\subsubsection*{Step 2: Initialize Parameters}
\[
q_{L,\max} = \frac{106.57 + 82.97 + 156.60 + 156.58 + 160.41}{5}
= 132.63
\]

Initial guesses:
\[
a = 0.2, \quad b = 0.5, \quad C_g = 50
\]


\subsubsection*{Step 3: Pressure Ratio (First Data Point)}
\[
p_r = \frac{P_{wf}}{P_{\text{res}}} = \frac{140.80}{295.44} \approx 0.48
\]


\subsubsection*{Step 4: Liquid Rate Prediction}
\[
q_L = q_{L,\max} (1 - a p_r - b p_r^2)
\]

\[
q_L = 132.63 \left(1 - 0.2 \times 0.48 - 0.5 \times 0.48^2 \right)
\]

\[
= 132.63 \times (1 - 0.096 - 0.115)
= 132.63 \times 0.789
= 104.63
\]
...
\subsubsection*{Step 5: Water Cut and Phase Split}
In this formulation, water cut is treated as an unknown parameter and initialized using a prior estimate. Assume an initial water cut:

\[
wc^{(0)} = 0.01
\]

The phase split is derived from the predicted liquid rate:
...
\[
q_w = 0.01 \times 104.63 = 1.046
\]
\[
q_o = 104.63 - 1.046 = 103.58
\]

Errors relative to well test:

\[
\epsilon_w = 1.046 - 0.0106 \approx +1.035
\]
\[
\epsilon_o = 103.58 - 106.56 \approx -2.98
\]

\noindent
\textbf{Observation:} The assumed water cut overestimates the water phase, indicating that the initial guess requires calibration.

\subsubsection*{Step 6: Gas Rate Prediction}
\[
\Delta p = \sqrt{P_{\text{res}} - P_{wf}} = \sqrt{295.44 - 140.80}
= \sqrt{154.64} \approx 12.43
\]

\[
\text{choke}_{\text{eff}} = 0.528
\]

\[
q_g = C_g \cdot \Delta p \cdot \text{choke}_{\text{eff}}
= 50 \times 12.43 \times 0.528 \approx 327.9
\]
...
\[
\epsilon_g \approx -12628.46
\]


\subsubsection*{Step 7: Residual Vector}
For this data point:

\[
\epsilon_o \approx -2.98, \quad
\epsilon_w \approx +1.035, \quad
\epsilon_g \approx -12628.46
\]

Residuals across all data points are:

\begin{itemize}
\item Aggregated into a residual vector
\item Normalized to account for scale differences
\item Used to update model parameters, including water cut
\end{itemize}

\subsubsection*{Step 8: Optimization Iteration}
Water cut is updated during calibration:
...
\[
q_w = 0.002 \times 104.63 = 0.209
\]
\[
q_o = 104.63 - 0.209 = 104.42
\]

Improved agreement with observed data indicates convergence toward the true phase fraction. Simultaneously, model parameters are updated:

\[
q_{L,\max} = 140, \quad a = 0.25, \quad b = 0.45
\]

\[
q_L = 140 \left(1 - 0.25 \cdot 0.48 - 0.45 \cdot 0.48^2 \right)
= 108.64
\]


\subsubsection*{Step 9: Final Calibrated Parameters}
\begin{table}[!htb]
\centering
\begin{tabular}{cc}
\hline
...
$a$ & 0.27 \\
$b$ & 0.43 \\
$C_g$ & scaled to match gas flow \\
$wc$ & $< 0.001$ \\
\hline
\end{tabular}
\caption{Final calibrated parameters including water cut}
\end{table}
...
The use of residual learning instead of direct rate prediction provides several advantages \citep{he2016deep}:
...
\begin{itemize}
    ...
    \item \textbf{Enhanced scalability:}  
        ...
        machine-learning residual model is shared across all wells. This design notably
...
Residual learning is performed in logarithmic space to stabilize variance and ensure positivity of predicted flow rates \citep{box1964analysis}. The residual targets are defined as
...
Prior to training, the feature matrix is standardized using zero-mean and unit-variance scaling \citep{hastie2009elements},
\begin{equation}
...
The function $f_{\text{ML}}(\cdot)$ is implemented as a \textit{MultiOutputRegressor} with gradient-boosted decision trees as base learners. Specifically, \textit{HistGradientBoostingRegressor} a histogram-based gradient boosting regressor is employed due to its robustness, fast training, and suitability for tabular industrial data \citep{friedman2001greedy}. Key hyperparameters are selected to balance accuracy and computational efficiency, including shallow tree depth, early stopping, and minimum leaf size constraints.
...
This formulation corresponds to a multiplicative correction \citep{box1964analysis} in the original rate
domain and reflects the assumption that modeling errors scale with the magnitude of the predicted flow rate.
...
The overall modeling workflow of the proposed hybrid framework consists of a sequence of structured steps that integrate physics-based modeling with data-driven residual learning. The pipeline is designed to ensure causal consistency, scalability across wells, and robustness under both temporal and spatial generalization settings.

\begin{enumerate}

\item \textbf{Data preparation and preprocessing:}  
Raw well data, including pressure, temperature, choke settings, and well-test measurements, are first cleaned and filtered to remove invalid or missing entries. Relevant features are normalized and aligned temporally. Well-test rates are treated as the reference ground truth for model training and evaluation.

\item \textbf{Temporal data partitioning:}  
For each well, the dataset is split into training, validation, and test sets using a lag-safe temporal splitting strategy. This ensures that future information is not leaked into the training phase and preserves causal consistency. For spatial generalization, a LOWO strategy is employed, where one well (e.g., W10) is held out entirely for testing.

\item \textbf{Physics model calibration:}  
A physics-based multiphase flow model is calibrated independently for each well using nonlinear least squares optimization. The calibration process estimates well-specific parameters such as reservoir pressure and flow coefficients, enabling the model to capture fundamental physical behavior.

\item \textbf{Physics-based prediction:}  
Using the calibrated parameters, the physics model is used to generate baseline predictions for oil, gas, and water flow rates across all samples. These predictions serve as the foundation for subsequent residual learning.

\item \textbf{Residual computation:}  
Residuals are computed as the difference between observed well-test measurements and physics-based predictions. These residuals represent systematic discrepancies arising from unmodeled physical effects, measurement noise, or simplifying assumptions in the physics model.

\item \textbf{Feature engineering for residual learning:}  
A reduced feature set is constructed using selected operational variables (e.g., pressure, choke) and physics-derived quantities. Additional transformations, such as log-space representation and regime-based grouping, are applied to improve learning stability and capture nonlinear relationships.

\item \textbf{Residual model training:}  
A machine learning model is trained globally across multiple wells to predict residual corrections. The use of pooled data enables the model to learn shared patterns in physics deviations, improving generalization across wells.

\item \textbf{Hybrid prediction:}  
Final predictions are obtained by combining physics-based estimates with the predicted residual corrections. Gating mechanisms and physical constraints are applied to ensure that corrected predictions remain within physically plausible limits.

\item \textbf{Model evaluation:}  
The hybrid model is evaluated using well-test measurements as the reference ground truth. Performance is assessed under both temporal and spatial generalization settings using metrics such as $R^2$, MAE, RMSE, and MAPE. Additionally, model performance is compared against MPFM estimations, which serve as an industry-standard benchmark.

\item \textbf{Model persistence:}  
Calibrated physics parameters and trained residual models are stored to enable reuse across wells and deployment scenarios. This avoids the need for repeated full retraining and ensures consistency in model application.

\item \textbf{Deployment:}  
For a new or unseen well, the physics-based model is calibrated using a limited set of well-test measurements to capture well-specific characteristics. The globally trained residual model is then applied without retraining, enabling efficient commissioning with minimal data requirements.

\item \textbf{Operation:}  
During operation, the hybrid model generates flow rate predictions at discrete time intervals using available operational inputs such as pressure, temperature, and choke settings. This supports near real-time production monitoring in practical field environments.
\end{enumerate}
...
At inference time, the hybrid model requires a single forward evaluation of the physics equations followed by a lightweight residual prediction. This results in low-latency predictions suitable for near real-time applications. Compared to purely data-driven deep learning models, the proposed approach offers considerably reduced inference cost, making it amenable to deployment in production monitoring systems with limited computational resources.
...
\begin{enumerate}
    \item Quantitatively evaluate the accuracy of oil, water, and gas flow rate predictions produced by the proposed hybrid physics-machine learning model using well-test measurements as reference data. Well test rates are treated as the primary source of truth (gold standard), as they provide the most reliable direct measurements of production rates, although they may still contain operational uncertainties. Assess temporal generalization by evaluating model performance on unseen time periods.
    ...
    \item Compare the predictive performance of the proposed modeling approaches against MPFM, in order to assess the added value of the data-driven framework relative to the industry standard MPFM benchmark.

    \item Assess the spatial generalization capability of the proposed framework under realistic deployment conditions by evaluating model performance on previously unseen wells using a LOWO strategy. For unseen wells, limited physics-model calibration is permitted, while the global machine-learning residual model is applied without retraining.

    \item Benchmark the predictive performance of the proposed hybrid framework against the results reported by \citep{nemoto2023cloud}, leveraging the fact that both studies are based on the wells from the same oil and gas field. This comparison aims to contextualize the performance of the proposed approach relative to existing cloud-based virtual flow metering methodologies under similar data.
\end{enumerate}
...
Comparing these configurations allows the incremental contribution of residual learning to be quantified. In addition, the performance of both experimental configurations is evaluated against the multiphase flow meter (MPFM) measurements, which serve as a strong industry baseline for production rate estimation.
...
The cross-well temporal generalization evaluation is designed to assess the ability of the proposed modeling framework to produce accurate flow-rate predictions on unseen time periods while remaining within the same set of wells. 
This evaluation reflects a common production monitoring scenario in which historical data are available for a well, and the objective is to predict flow rates under future operating conditions without retraining the model.
...
To ensure robust model development and unbiased performance evaluation, the dataset is partitioned into training, validation, and test subsets on a per-well basis using a temporal hold-out strategy.

For each well, observations are first sorted chronologically, and the data are then split into three contiguous segments according to predefined proportions:

\begin{itemize}
\item \textbf{Training set}: 70% of the earliest observations
\item \textbf{Validation set}: 15% of subsequent observations
\item \textbf{Test set}: 15% of the most recent observations
\end{itemize}

To further mitigate temporal leakage and autocorrelation effects between subsets, a fixed gap is introduced between consecutive splits. Specifically, a small number of samples separating the training–validation and validation–test boundaries are excluded from all subsets. This ensures that short-term temporal dependencies do not artificially inflate model performance.

Unlike random sampling approaches, this method strictly preserves the temporal ordering of observations, thereby reflecting real-world deployment conditions where models are trained on historical data and evaluated on future unseen data.

The partitioning is performed independently for each well to ensure that:
(i) all wells are represented across the training, validation, and test sets, and
(ii) well-specific temporal dynamics are preserved without inter-well data leakage.

Wells with insufficient data to satisfy the minimum split requirements (including the gap constraints) are excluded from the analysis to maintain the integrity of the evaluation.

The final datasets are obtained by concatenating the corresponding subsets across all wells. This strategy provides a realistic and physically consistent evaluation framework for time-dependent production data, while avoiding information leakage between training and evaluation phases.
...
Temporal generalization evaluates the ability of the proposed model to predict future production behavior using historical data from the same well. This is a critical requirement for virtual flow metering applications, where models must remain reliable under evolving operating conditions, including changes in pressure, choke settings, and multiphase flow regimes.

In this study, temporal generalization is assessed by training the models on earlier time periods and evaluating them on unseen future data for each well. This setup ensures that no future information is used during training, thereby mimicking real-world deployment conditions. The evaluation is conducted across multiple wells with varying flow characteristics, enabling a comprehensive assessment of model robustness and adaptability.

The performance of three approaches is compared: the physics-based model, the proposed hybrid physics-machine learning model, and the MPFM baseline. Metrics including $R^2$, MAE, RMSE, and MAPE are used to quantify predictive accuracy and reliability across wells.


\subsection{Gas Rate (\texorpdfstring{$q_g$}{qg})}
Table~\ref{tab:temporal_gas_rate_comparison} reports gas rate prediction performance under lag-safe temporal extrapolation.

The physics-based model exhibits highly variable performance for gas rate prediction across wells. While moderate accuracy is achieved for wells such as W06 ($R^2 = 0.77$) and W11 ($R^2 = 0.59$), the model fails significantly for several others, including W08, W15, and W19, where strongly negative $R^2$ values are observed. These results indicate that the physics formulation struggles to capture the complex gas flow behavior under varying operating conditions, particularly in wells with dynamic pressure drawdown and multiphase interactions. The consistently high MAE and RMSE values further highlight the limited predictive capability of the standalone physics model for temporal extrapolation.

In contrast, the hybrid model demonstrates consistently strong performance across all wells, as shown in table~\ref{tab:temporal_gas_rate_comparison}. High predictive accuracy is achieved for most wells, with $R^2$ values exceeding 0.85 and reaching near-perfect levels for W06, W11, and W18. The substantial reduction in MAE and RMSE compared to the physics model confirms the effectiveness of the residual learning framework in correcting systematic biases in gas rate predictions. Notably, even for challenging wells such as W08, where the physics model fails ($R^2 = -30.24$), the hybrid model achieves a positive $R^2$ of 0.44, demonstrating its ability to generalize across difficult operating regimes. The consistently low MAPE values further indicate stable and reliable predictive performance.

The MPFM baseline generally provides strong performance for gas rate prediction, achieving high $R^2$ values for wells such as W06, W10, W11, and W15. However, significant degradation is observed for certain wells, particularly W08 and W18, where negative $R^2$ values and large prediction errors are recorded. This suggests that MPFM measurements are sensitive to flow conditions and may become unreliable under complex multiphase regimes or adverse operating conditions.

Overall, the hybrid model achieves the best balance between accuracy and robustness. It consistently outperforms the physics-based model and demonstrates more stable performance than the MPFM baseline across wells. These results confirm that integrating physics-based modeling with machine learning residual correction significantly enhances temporal generalization capability for gas rate prediction.

\begin{table}[!ht]
\centering
\caption{Temporal generalization performance for gas rate ($q_g$)}
\label{tab:temporal_gas_rate_comparison}
\begin{tabular}{lccccccc}
\hline
\hline
\multicolumn{8}{c}{Physics Model} \\
\hline
Metric & W06 & W08 & W10 & W11 & W15 & W18 & W19 \\
\hline
$R^2$ & 0.77 & -30.24 & -0.06 & 0.59 & -7.23 & -0.10 & -23.74 \\
MAE & 1823.01 & 3186.58 & 3163.45 & 2310.10 & 3115.71 & 6685.97 & 6520.88 \\
RMSE & 2204.70 & 3227.35 & 3867.35 & 2937.40 & 4070.56 & 7147.10 & 6579.77 \\
MAPE (\%) & 13.67 & 78.41 & 24.47 & 21.01 & 50.32 & 55.01 & 89.95 \\
\hline

\multicolumn{8}{c}{Hybrid Model} \\
\hline
$R^2$ & 0.99 & 0.44 & 0.94 & 0.97 & 0.79 & 0.95 & 0.85 \\
MAE & 96.74 & 374.89 & 447.92 & 385.44 & 402.05 & 614.78 & 540.34 \\
RMSE & 119.94 & 378.78 & 640.71 & 444.37 & 686.43 & 749.10 & 549.49 \\
MAPE (\%) & 0.95 & 9.62 & 3.21 & 5.83 & 6.77 & 4.34 & 7.53 \\
\hline

\multicolumn{8}{c}{MPFM} \\
\hline
$R^2$ & 0.99 & -12.95 & 0.92 & 0.93 & 0.96 & -1.71 & 0.94 \\
MAE & 330.88 & 1699.02 & 877.14 & 809.32 & 252.92 & 9630.87 & 311.74 \\
RMSE & 366.27 & 1890.10 & 1078.73 & 1244.23 & 299.93 & 11138.26 & 343.36 \\
MAPE (\%) & 2.38 & 45.25 & 6.12 & 13.63 & 4.64 & 46.58 & 4.07 \\
\hline
\end{tabular}
\end{table}

\subsection{Oil Rate ($q_o$)}
Table~\ref{tab:temporal_oil_rate_comparison} summarizes the per-well temporal generalization performance for oil rate prediction.

The physics-based model exhibits highly inconsistent performance across wells. While it achieves moderate accuracy for well W06 ($R^2 = 0.65$), it fails significantly for most other wells, with strongly negative $R^2$ values (e.g., W08, W11, W15, W19). This indicates that the physics formulation alone is unable to capture the complex and evolving dynamics of multiphase flow under changing operating conditions. The large errors in MAE and RMSE further confirm the limited adaptability of the physics model to temporal variations.

In contrast, the proposed hybrid model demonstrates consistently strong performance across the majority of wells. Near-perfect predictive accuracy is observed for several wells, including W06, W10, W11, and W18, with $R^2$ values close to 1.0 and very low MAE and RMSE. This highlights the effectiveness of residual learning in correcting systematic biases in the physics-based predictions. Even for more challenging wells such as W15 and W19, the hybrid model maintains reasonable performance, significantly outperforming the physics model.

However, performance degradation is observed for well W08 ($R^2 = -1.87$), indicating that the hybrid model may struggle under certain operating regimes not sufficiently represented in the training data. This suggests that the effectiveness of the residual learning component is dependent on the coverage of flow conditions and regimes seen during training.

The MPFM baseline generally provides strong performance, particularly for wells such as W06, W10, and W15. However, it exhibits instability for certain wells, notably W08 and W18, where negative $R^2$ values and high errors are observed. This highlights the sensitivity of MPFM measurements to flow conditions and potential measurement uncertainties.

Overall, the hybrid model achieves the best balance between accuracy and robustness. It consistently outperforms the physics-based model and demonstrates competitive, and in many cases superior, performance compared to MPFM. These results confirm that integrating physics-based modeling with machine learning residual correction considerably enhances temporal generalization capability for oil rate prediction.

\begin{table}[!ht]
\centering
\caption{Temporal generalization performance for oil rate ($q_o$)}
\label{tab:temporal_oil_rate_comparison}
\begin{tabular}{lccccccc}
\hline
\hline
\multicolumn{8}{c}{Physics Model} \\
...
Metric & W06 & W08 & W10 & W11 & W15 & W18 & W19 \\
\hline
$R^2$ & 0.65 & -11.39 & -2.28 & -10.76 & -18.77 & -8.86 & -32.68 \\
MAE & 18.53 & 9.74 & 47.63 & 81.93 & 38.66 & 105.13 & 58.87 \\
RMSE & 22.63 & 11.84 & 67.75 & 139.10 & 41.75 & 112.39 & 62.59 \\
MAPE (\%) & 25.09 & 86.45 & 61.31 & 108.27 & 138.13 & 217.45 & 173.15 \\
\hline
\multicolumn{8}{c}{Hybrid Model} \\
\hline
$R^2$ & 0.99 & -1.87 & 0.98 & 0.98 & 0.73 & 0.98 & 0.85 \\
MAE & 0.49 & 2.42 & 2.46 & 3.51 & 3.46 & 4.43 & 3.71 \\
RMSE & 0.67 & 2.43 & 3.85 & 4.46 & 4.65 & 4.93 & 3.94 \\
MAPE (\%) & 0.61 & 22.04 & 2.69 & 5.01 & 11.46 & 7.89 & 11.40 \\
\hline
\multicolumn{8}{c}{MPFM} \\
\hline
$R^2$ & 0.99 & -7.08 & 0.99 & 0.85 & 0.95 & -0.72 & 0.86 \\
MAE & 1.45 & 3.84 & 2.56 & 8.71 & 1.84 & 44.54 & 2.85 \\
RMSE & 1.89 & 4.07 & 3.32 & 16.10 & 2.08 & 47.53 & 4.27 \\
MAPE (\%) & 1.16 & 35.45 & 2.84 & 20.15 & 7.76 & 61.38 & 6.86 \\
\hline
\end{tabular}
\end{table}

\subsection{Water Rate ($q_w$)}
Table~\ref{tab:temporal_water_rate_comparison} reports water rate prediction performance under lag-safe temporal extrapolation.

The physics-based model performs poorly for water rate prediction across all wells, with consistently negative $R^2$ values. This indicates that the model fails to capture the underlying dynamics of water production, particularly under multiphase flow conditions where water behavior is highly nonlinear and intermittent. Although the absolute errors (MAE and RMSE) are relatively moderate for some wells, the persistently high MAPE values often approaching or exceeding 100\% - highlight the difficulty of accurately modeling water rates using physics alone. These results suggest that the simplified physics formulation is insufficient to represent complex water production mechanisms such as phase interactions, breakthrough events, and low-rate regimes.

In contrast, the hybrid model demonstrates substantial improvement in water rate prediction for most wells, as shown in table~\ref{tab:temporal_water_rate_comparison}. Strong performance is observed for wells such as W06, W10, W11, and W15, where positive $R^2$ values and significantly reduced MAE and RMSE indicate accurate and stable predictions. The hybrid model effectively leverages residual learning to correct systematic biases in the physics model, resulting in consistent performance gains across varying operating conditions. However, reduced performance is observed for wells such as W08 and W18, where $R^2$ remains low or slightly negative. This suggests that water prediction remains challenging in certain regimes, particularly where water production is highly sparse or exhibits abrupt transitions.

The MPFM baseline shows mixed performance for water rate prediction. While it achieves high accuracy for certain wells (e.g., W10), it performs poorly for others, including W06 and W18, where negative $R^2$ values are observed. This variability reflects the sensitivity of MPFM measurements to flow conditions, especially in low water-cut or unstable multiphase environments. Although MPFM provides reasonable MAE and RMSE values in some cases, its overall performance lacks consistency across wells.

It is important to note that MAPE is less reliable for water rate evaluation due to the presence of near-zero flow conditions. Small absolute errors can result in disproportionately large percentage errors, inflating MAPE values even when predictions are reasonably accurate in absolute terms. Therefore, MAE and RMSE provide a more meaningful assessment of model performance for water rates.

Overall, the hybrid model provides the most balanced and reliable performance for water rate prediction. While water remains the most challenging phase to model, the proposed framework indicatively improves prediction accuracy compared to both the physics-based model and the MPFM baseline, demonstrating the effectiveness of physics-informed residual learning in capturing complex multiphase flow behavior.

\begin{table}[!ht]
\centering
\caption{Temporal generalization performance for water rate ($q_w$)}
\label{tab:temporal_water_rate_comparison}
\begin{tabular}{lccccccc}
...
Metric & W06 & W08 & W10 & W11 & W15 & W18 & W19 \\
\hline
$R^2$ & -2.46 & -25.01 & -0.74 & -3.49 & -3.12 & -22.08 & -15.86 \\
MAE & 2.35 & 15.05 & 19.54 & 21.81 & 5.88 & 52.93 & 20.41 \\
RMSE & 2.78 & 15.71 & 30.02 & 24.58 & 6.75 & 54.21 & 21.06 \\
MAPE (\%) & 100.00 & 38.12 & 99.73 & 97.15 & 100.00 & 93.06 & 90.89 \\
\hline

\multicolumn{8}{c}{Hybrid Model} \\
\hline
$R^2$ & 0.97 & -3.89 & 0.84 & 0.83 & 0.84 & -0.11 & 0.40 \\
MAE & 0.20 & 6.29 & 5.41 & 4.28 & 0.95 & 11.75 & 3.85 \\
RMSE & 0.25 & 6.35 & 9.06 & 4.92 & 1.17 & 12.23 & 4.12 \\
MAPE (\%) & 20.57 & 16.33 & 23.72 & 18.25 & 23.01 & 19.99 & 16.22 \\
\hline

\multicolumn{8}{c}{MPFM} \\
\hline
$R^2$ & -0.98 & 0.18 & 0.98 & 0.19 & 0.10 & -2.88 & 0.08 \\
MAE & 1.67 & 2.35 & 2.54 & 7.65 & 2.11 & 22.48 & 2.59 \\
RMSE & 2.06 & 2.60 & 3.10 & 10.76 & 2.78 & 22.84 & 5.09 \\
MAPE (\%) & 69.96 & 5.90 & 82.23 & 34.02 & 47.85 & 39.62 & 19.06 \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[!htb]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W06_qg_hybrid.png}}{}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \uomFig
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W06_qo_hybrid.png}}{}
    \end{subfigure}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W06_qw_hybrid.png}}{}
    \end{subfigure}

    \caption{Temporal generalization performance of Physics, Hybrid, and MPFM models for W06.
...
\begin{figure}[!htb]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W08_qg_hybrid.png}}{}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \uomFig
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W08_qo_hybrid.png}}{}
    \end{subfigure}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W08_qw_hybrid.png}}{}
    \end{subfigure}

    \caption{Temporal generalization performance of Physics, Hybrid, and MPFM models for W08.
    The plots show predicted and measured gas, oil, and water rates over time.}
...
\begin{figure}[!htb]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W10_qg_hybrid.png}}{}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \uomFig
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W10_qo_hybrid.png}}{}
    \end{subfigure}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W10_qw_hybrid.png}}{}
    \end{subfigure}

    \caption{Temporal generalization performance of Physics, Hybrid, and MPFM models for W10.
    The plots show predicted and measured gas, oil, and water rates over time.}
...
\begin{figure}[!htb]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W11_qg_hybrid.png}}{}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \uomFig
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W11_qo_hybrid.png}}{}
    \end{subfigure}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W11_qw_hybrid.png}}{}
    \end{subfigure}

    \caption{Temporal generalization performance of Physics, Hybrid, and MPFM models for W11.
    The plots show predicted and measured gas, oil, and water rates over time.}
...
\begin{figure}[!htb]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W18_qg_hybrid.png}}{}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \uomFig
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W18_qo_hybrid.png}}{}
    \end{subfigure}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W18_qw_hybrid.png}}{}
    \end{subfigure}

    \caption{Temporal generalization performance of Physics, Hybrid, and MPFM models for W18.
    The plots show predicted and measured gas, oil, and water rates over time.}
...
\begin{figure}[!htb]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W19_qg_hybrid.png}}{}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \uomFig
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W19_qo_hybrid.png}}{}
    \end{subfigure}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        {\includegraphics[width=\textwidth]{images/finalized_results/W19_qw_hybrid.png}}{}
    \end{subfigure}

    \caption{Temporal generalization performance of Physics, Hybrid, and MPFM models for W19.
    The plots show predicted and measured gas, oil, and water rates over time.}
...
This section evaluates the ability of the proposed model to generalize across wells, i.e., to predict production rates for wells that were not observed during training. This is assessed using the Leave-One-Well-Out (LOWO) evaluation protocol described in Section~\ref{sec:lowo_setup}, where all data from a single well is held out for testing, and the model is trained on the remaining wells. In this study, well W10 is selected as the held-out well for evaluation.

Spatial generalization represents a more challenging task than temporal generalization, as it requires the model to transfer learned relationships across wells with potentially different geological characteristics, flow regimes, and operational conditions. Therefore, strong performance under LOWO evaluation is indicative of a model's robustness and practical applicability in real-world deployment scenarios.

\subsection{LOWO Results}
Table~\ref{tab:lowo} summarizes the spatial generalization performance for the held-out well W10.

The physics-based model demonstrates limited generalization capability across all phases. For oil rate prediction, the model achieves a low $R^2$ of 0.07, indicating that it captures only a small fraction of the variance in the unseen well. Similarly, for gas rate prediction, although a moderate $R^2$ of 0.32 is observed, the absolute errors remain high, reflecting poor predictive accuracy. Water rate prediction is particularly poor, with a negative $R^2$ value, confirming the inability of the physics model to generalize across wells with different multiphase flow characteristics. These results highlight the inherent limitations of relying solely on physics-based formulations, which are often calibrated to well-specific conditions and lack adaptability to new wells.

In contrast, the hybrid model demonstrates excellent spatial generalization performance across all phases. For oil and gas rates, near-perfect accuracy is achieved, with $R^2$ values of 0.99 and substantial reductions in MAE and RMSE compared to both the physics model and the MPFM baseline. This indicates that the hybrid framework successfully captures transferable patterns across wells by combining physics-based predictions with data-driven residual corrections. The strong performance on the unseen well W10 suggests that the learned residual relationships are not merely overfitted to specific wells, but instead generalize effectively to new operating conditions.

For water rate prediction, the hybrid model also shows significant improvement over the physics model, achieving a high $R^2$ of 0.93 and low absolute errors. However, the MAPE value is notably large, which is primarily due to the presence of near-zero water rates. In such cases, small absolute errors can lead to disproportionately large percentage errors, making MAPE an unreliable metric for evaluating water rate performance. Therefore, MAE and RMSE provide a more meaningful assessment of model accuracy for this phase.

The MPFM baseline generally provides strong performance, particularly for oil and gas rates, with high $R^2$ values and relatively low errors. However, its performance is less consistent compared to the hybrid model, especially for water rates and under certain flow conditions. While MPFM measurements can be accurate, they are inherently dependent on sensor quality and flow regime assumptions, which may limit their reliability in complex multiphase environments.

Overall, the hybrid model achieves the best balance between accuracy and generalization capability. It significantly outperforms the physics-based model and demonstrates competitive or superior performance compared to MPFM across all phases. These results confirm that the integration of physics-based modeling with machine learning enables effective knowledge transfer across wells, making the proposed framework a robust solution for spatial generalization in virtual flow metering applications.


\begin{table}[!ht]
\centering
\caption{Spatial generalization (LOWO) performance for held-out well W10}
\label{tab:lowo}
\begin{tabular}{lccccc}
\hline
Quantity & Model & $R^2$ & MAE & RMSE & MAPE (\%) \\
\hline
$q_o$ & Physics & 0.07 & 20.39 & 29.67 & 20.36 \\
      & Hybrid  & 0.96 & 1.60  & 2.14  & 1.53 \\
      & MPFM    & 0.97 & 3.07  & 5.55  & 2.82 \\
\hline
$q_g$ & Physics & 0.32 & 2439.63 & 3050.20 & 19.30 \\
      & Hybrid  & 0.94 & 252.94  & 379.06  & 2.24 \\
      & MPFM    & 0.95 & 499.48  & 867.57  & 3.46 \\
\hline
$q_w$ & Physics & -0.25 & 5.39 & 12.15 & 100.00 \\
      & Hybrid  & 0.93  & 0.93 & 2.98  & 856.08 \\
      & MPFM    & 0.93  & 2.42 & 2.87  & 90809.43 \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[!htb]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \uomFig
        {W10 gas rate spatial generalization performance}
        {fig:W10_qg_spatial_generalization_model_performance}
        {\includegraphics[width=\textwidth]{images/finalized_results/W10_qg_lowo_hybrid.png}}{}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \uomFig
        {W10 oil rate spatial generalization performance}
        {fig:W10_qo_spatial_generalization_model_performance}
        {\includegraphics[width=\textwidth]{images/finalized_results/W10_qo_lowo_hybrid.png}}{}
    \end{subfigure}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \uomFig
        {W10 water rate spatial generalization performance}
        {fig:W10_qw_spatial_generalization_model_performance}
        {\includegraphics[width=\textwidth]{images/finalized_results/W10_qw_lowo_hybrid.png}}{}
    \end{subfigure}

    \caption{Spatial generalization performance of Physics, Hybrid, and MPFM models for W10.
    The plots show predicted and measured gas, oil, and water rates over time.}
    \label{fig:W10_spatial_generalization}
\end{figure}

\section{Performance Comparison with Contemporary Hybrid VFM}
Table~\ref{tab:wgr_comparison} presents a comparison of the proposed hybrid model against the WGR prediction models reported in \citep{nemoto2023cloud}.
It is important to note that while both this study and \citep{nemoto2023cloud} are based on wells from the same offshore gas field, the specific wells used in each study are not necessarily identical. As a result, the comparison reflects performance across similar field conditions rather than an exact well-to-well evaluation. This distinction is important, as variability in well behavior, operating conditions, and flow regimes can influence model performance.

The proposed hybrid model demonstrates superior overall predictive performance compared to the literature models. In particular, it achieves the highest coefficient of determination ($R^2 = 0.90$), indicating a stronger ability to capture the variability in WGR across the dataset. In contrast, the models reported in \citep{nemoto2023cloud} achieve lower $R^2$ values ranging from 0.64 to 0.80, suggesting comparatively weaker predictive capability.

In terms of error metrics, the hybrid model achieves a MAPE of 20.29\%, which is comparable to the literature models (17.43\%--20.61\%). While one of the literature configurations reports a slightly lower MAPE (17.43\%), this comes at the expense of a lower $R^2$ value (0.80), indicating that the model may not capture the overall variability as effectively. The proposed model, on the other hand, provides a better balance between accuracy and explanatory power.

The MRE (approximated using MPE) for the hybrid model is negative (-11.48\%), indicating a tendency toward slight underprediction. In comparison, the literature models exhibit both positive and near-zero MRE values, reflecting varying degrees of bias depending on the modeling assumptions. This highlights that while the hybrid model improves overall accuracy, some systematic bias may still remain and could be further refined.

A key distinction between the proposed approach and the literature models lies in the modeling methodology. The models in \citep{nemoto2023cloud} rely on different combinations of flow correlations and tuned friction factors, whereas the proposed hybrid framework integrates physics-based modeling with data-driven residual correction. The improved performance of the hybrid model suggests that learning residual corrections enables the model to capture complex deviations from simplified physical assumptions, leading to enhanced predictive capability.

Given that both studies are conducted on wells from the same field, the observed performance improvements are particularly meaningful, as they indicate that the proposed hybrid approach generalizes effectively within the same asset environment despite differences in individual well characteristics. This strengthens the validity of the comparison and highlights the robustness of the proposed framework.

Overall, the results indicate that the proposed hybrid model achieves state-of-the-art performance for WGR prediction, outperforming existing physics-based approaches reported in the literature while maintaining comparable error levels. This demonstrates the effectiveness of combining physical modeling with machine learning for improved virtual flow metering performance.

\begin{table}[!ht]
\centering
\caption{Comparison of global WGR prediction performance with contemporary hybrid VFM}
\label{tab:wgr_comparison}
\begin{tabular}{lccc}
\hline
Model & MRE (\%) & MAPE (\%) & $R^2$ \\
\hline
Hybrid (Proposed) & -11.48 & 20.29 & 0.89 \\
\hline
No-slip liquid holdup, Colebrook friction factor\textsuperscript{1} & 12.64 & 20.61 & 0.77 \\
No-slip liquid holdup, tuned friction factor\textsuperscript{1} & 0.43 & 17.43 & 0.80 \\
Slip liquid holdup, tuned friction factor\textsuperscript{1} & -0.23 & 20.10 & 0.64 \\
\hline
\end{tabular}
...
\begin{flushleft}
\footnotesize
\textsuperscript{1} Results reported in \citep{nemoto2023cloud} \\
\end{flushleft}
\end{table}

\section{Key Observations}
The following key observations can be drawn from the temporal and spatial generalization experiments:

\begin{itemize}

\item \textbf{Physics-only models lack generalization capability.}  
The physics-based model consistently underperforms in both temporal and spatial settings, with frequent negative $R^2$ values. This indicates that simplified physics formulations are insufficient to capture complex, evolving multiphase flow dynamics, especially under changing operating conditions and across different wells.

\item \textbf{Hybrid model improves predictive accuracy.}  
The proposed hybrid physics machine learning model consistently achieves the lowest errors and highest $R^2$ values across oil, gas, and water rates. The integration of residual learning effectively corrects systematic biases in the physics model, leading to substantial performance gains.

\item \textbf{Strong temporal generalization across most wells.}  
The hybrid model demonstrates near-perfect temporal prediction performance for oil and gas rates in multiple wells, indicating its ability to adapt to evolving production conditions over time. Performance degradation is limited to a small number of challenging wells (e.g., W08), suggesting sensitivity to underrepresented operating regimes.

\item \textbf{Effective spatial generalization to unseen wells (LOWO).}  
Under the LOWO evaluation (with W10 as the held-out well), the hybrid model maintains high accuracy for both oil and gas rates ($R^2 \approx 0.99$), demonstrating strong transferability of learned relationships across wells. This confirms that the model captures underlying physical patterns rather than overfitting to well-specific behavior.

\item \textbf{Water rate prediction remains inherently challenging.}  
All models exhibit relatively lower and more variable performance for water rate prediction. Although the hybrid model notably improves accuracy compared to the physics model, performance remains sensitive to sparse and intermittent water production patterns.

\item \textbf{MAPE is unreliable for low-rate conditions.}  
Extremely high MAPE values observed for water rates are primarily due to near-zero flow conditions, where small absolute errors lead to disproportionately large percentage errors. Therefore, MAE and RMSE provide a more reliable assessment of model performance for water prediction.

\item \textbf{MPFM provides a strong but inconsistent baseline.}  
MPFM measurements generally achieve high accuracy for oil and gas rates but exhibit instability under certain conditions (e.g., specific wells or flow regimes). This highlights limitations related to sensor reliability and flow regime assumptions in multiphase environments.

\item \textbf{Hybrid model offers the best balance between accuracy and robustness.}  
Across both temporal and spatial evaluations, the hybrid model consistently outperforms the physics-based model and demonstrates more stable performance than MPFM. This confirms the effectiveness of combining physics-based modeling with machine learning for virtual flow metering applications.

\item \textbf{Hybrid model outperforms literature physics-based approaches.}  
Comparison with the models reported in \citep{nemoto2023cloud} shows that the proposed hybrid framework achieves higher $R^2$ values for WGR prediction, indicating improved ability to capture variability in multiphase flow behavior compared to traditional physics-based formulations.
While some model configurations achieve slightly lower MAPE values, they do so at the cost of reduced $R^2$. In contrast, the hybrid model maintains competitive error levels while achieving notably higher $R^2$, demonstrating a better balance between predictive accuracy and overall model fidelity.

\item \textbf{Residual learning enhances physics-based modeling.}  
The superior performance of the hybrid model compared to \citep{nemoto2023cloud} highlights the advantage of incorporating data-driven residual correction on top of physics-based predictions. This enables the model to capture complex deviations from simplified flow correlations and friction factor assumptions used in traditional methods.

\item \textbf{Consistent performance within the same field environment.}  
Although the wells used in this study and in \citep{nemoto2023cloud} are not necessarily identical, they originate from the same offshore gas field. The improved performance of the hybrid model under these conditions indicates its ability to generalize effectively within the same asset environment despite variations in individual well characteristics.

\item \textbf{Hybrid approach demonstrates state-of-the-art performance.}  
Overall, the proposed hybrid model achieves performance that is competitive with or superior to existing approaches reported in \citep{nemoto2023cloud}, establishing it as a robust and effective solution for WGR prediction in virtual flow metering applications.
...
This chapter interprets the experimental findings in the context of physical modeling, data-driven learning, and practical deployment considerations.

\section{Temporal vs Spatial Generalization}
The temporal generalization results demonstrate that the hybrid model is highly effective at extrapolating future behavior within the same well when lag-safe causality constraints are enforced. Across multiple wells, the hybrid model achieves near industry-standard accuracy for oil and gas rates, significantly outperforming the physics-based model and consistently matching or exceeding the performance of MPFM, which serves as an industry-standard benchmark. This indicates that the residual errors learned by the model are largely stable over time within a given well, enabling consistent correction of systematic physics bias under evolving operating conditions.

In contrast, spatial generalization is inherently more challenging, as it requires the model to transfer learned relationships across wells with different characteristics. Despite this, the hybrid model demonstrates strong transferability under the LOWO evaluation (with W10 as the held-out well), achieving near industry-standard accuracy for oil and gas rates. This suggests that the learned residual corrections capture underlying physical deviations that are consistent across wells, rather than being purely well-specific. The ability to generalize to an unseen well further confirms that the model is not overfitting to individual well behavior, but instead learning transferable representations of multiphase flow dynamics.

However, some performance variability is still observed across wells in the temporal setting (e.g., W08), highlighting the sensitivity of the model to underrepresented operating regimes. This indicates that while the hybrid model generalizes well, its performance is influenced by the diversity and coverage of training data. Compared to MPFM, the hybrid model provides more consistent performance across wells, whereas MPFM exhibits instability under certain flow conditions, reflecting its dependence on sensor assumptions and flow regime behavior.

\section{Comparison with Comtemporary Hybrid VFM}
The comparison with the models reported in \citep{nemoto2023cloud} provides additional context for evaluating the effectiveness of the proposed approach. While both studies are based on wells from the same offshore gas field, the specific wells used are not necessarily identical. Therefore, the comparison reflects performance under similar field conditions rather than a direct well-to-well evaluation.

Despite this, the proposed hybrid model demonstrates superior performance in terms of explanatory power, achieving higher $R^2$ values compared to the physics-based configurations presented in the \citep{nemoto2023cloud} model. Although some literature configurations achieve slightly lower MAPE values, these are typically associated with lower $R^2$, indicating a reduced ability to capture variability in WGR behavior.

The improved performance of the hybrid model can be attributed to its ability to learn residual corrections on top of physics-based predictions. In contrast, the models in \citep{nemoto2023cloud} rely on variations of flow correlations and tuned friction factors, which may not fully capture complex deviations from idealized physical assumptions. The results therefore suggest that augmenting physics-based models with data-driven residual learning provides a more flexible and accurate framework for virtual flow metering.

Importantly, the fact that both studies are conducted within the same field environment strengthens the validity of this comparison. The observed improvements indicate that the hybrid approach generalizes effectively within the same asset, even when applied to different wells with varying operational characteristics.

\section{Deployment Implications}
The results demonstrate that the hybrid framework offers a practical and scalable alternative to traditional MPFM systems, particularly in environments where dense instrumentation is unavailable or cost-prohibitive. While MPFM provides strong performance in many cases, it should be noted that it is not a true ground-truth measurement but rather an industry-standard benchmark subject to measurement uncertainty and flow regime limitations. As observed in the experiments, MPFM performance can degrade under complex multiphase conditions.

The hybrid model, on the other hand, achieves substantial improvements over physics-only approaches while maintaining competitive performance relative to MPFM. Its ability to generalize both temporally and spatially makes it well-suited for deployment in brownfield assets, where historical data is available but measurement infrastructure may be limited.

Furthermore, the use of lag-safe temporal splitting ensures that the evaluation reflects realistic deployment scenarios, where future data is not accessible during model training. This strengthens the validity of the reported results and demonstrates that the model can operate under causal constraints.

An additional advantage of the hybrid approach is its ability to leverage existing physics knowledge while adapting to data-driven corrections, reducing reliance on extensive sensor calibration and manual tuning required in traditional systems.

\section{Summary}
In summary, the experimental results confirm that:
\begin{itemize}
    \item Hybrid physics-ML modeling indicatively improves production rate estimation under both temporal and spatial generalization, particularly for oil and gas rates.
    \item The hybrid model demonstrates strong transferability across wells, achieving near-perfect performance under LOWO evaluation (W10) for unseen wells.
    \item Comparison with the \citep{nemoto2023cloud} model shows that the proposed approach achieves superior explanatory power while maintaining competitive error levels.
    \item Lag-safe data partitioning is essential for ensuring causal and scientifically valid evaluation of time-series models.
    \item Water rate prediction remains challenging due to sparse and intermittent flow behavior, and percentage-based metrics such as MAPE can be misleading in low-rate conditions.
    \item MPFM serves as a strong industry benchmark but exhibits variability under certain flow conditions, highlighting the need for robust data-driven alternatives.
    \item The hybrid approach provides a robust and practical alternative to both physics-only models and MPFM, offering improved consistency across varying operating conditions.
\end{itemize}

These findings support the central thesis that physics-informed residual learning can bridge the gap between deterministic models and fully data-driven approaches, enabling accurate, robust, and deployable solutions for real-world well production monitoring.
