...
\item Water cut ($wc$) is a key parameter describing liquid composition:
\[
wc = \frac{q_w}{q_L}, \quad q_L = q_o + q_w
\]
...
\subsection{Relevance to Virtual Flow Metering}

Multiphase flow measurement remains a major challenge due to the simultaneous presence of oil, gas, and water phases. Traditional approaches such as test separators provide accurate but infrequent measurements, motivating the development of virtual flow metering (VFM) techniques \citep{falcone2009multiphase}.

Hybrid approaches that combine physics-based modeling with data-driven techniques have shown improved performance in handling complex multiphase systems, as they leverage both physical understanding and data-driven adaptability \citep{kolev2016multiphase}.
...
% Wellhead vertical pipe
\draw[thick] (0,0) -- (0,1.2);
\node[above] at (0,1.2) {Wellhead};

% Surface sensors
\node[left] at (-0.2,1.0) {WHT};
\node[above left] at (-0.2,1.2) {WHP};
...
\draw (2,1.2) -- (2.3,1.5);
\draw (2,1.2) -- (2.3,0.9);
\node[below] at (2.2,1.0) {choke};

% Downstream pressure
\node[above] at (2.2,1.5) {DCP};

% Production arrow
\draw[arrow] (3,1.2) -- (4.5,1.2);
\node[right] at (4.5,1.2) {To Separator};

% Wellbore (smooth curve)
\draw[thick] (0,0) .. controls (-0.3,-1.5) and (-1.5,-2.5) .. (-2,-3.5);
\node at (-1.0,-1.8) {Wellbore};

% Reservoir
\draw[thick] (-3,-4) ellipse (2cm and 1cm);
\node at (-3,-4) {Well Downhole};

% Downhole sensors
\node[right] at (-1.3,-3.8) {DHP};
\node[right] at (-1.3,-4.2) {DHT};

\draw (-1.8,-3.8) circle (0.08);
\draw (-1.8,-4.2) circle (0.08);

\end{tikzpicture}
\caption{Well Downhole and Surface Instrumentation}
\label{fig:well-instrumentation}
\end{figure}


\begin{table}[!htb]
\uomTable{Well downhole and surface instrumentation}{tab:well-instrumentation}{\begin{tabularx}{0.9\textwidth}{lYYYY} 
\hline
