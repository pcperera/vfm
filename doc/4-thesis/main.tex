...
\begin{table}[!htb]
\centering
\caption{Calibrated parameters of the physics-based production model}
\label{tab:physics_calibrated_parameters}
\begin{tabular}{lll}
\hline
\textbf{Parameter} & \textbf{Scientific purpose} & \textbf{Unit} \\
\hline
$P_{\mathrm{res}}$ 
& Effective reservoir pressure governing inflow and drawdown behavior 
& bara \\

$q_{L,\max}$ 
& Maximum achievable liquid production rate at negligible bottomhole pressure 
& Sm$^{3}$/h \\

$a$ 
& Linear inflow coefficient controlling first-order pressure–rate sensitivity 
& -- \\

$b$ 
& Quadratic inflow coefficient accounting for nonlinear inflow effects 
& -- \\

$\boldsymbol{A}_{\mathrm{wc}}$ 
& Logistic water-cut model coefficients governing oil–water phase partitioning 
& -- \\

$C_{g}$ 
& Reservoir gas productivity coefficient relating pressure drawdown to gas rate 
& Sm$^{3}$/h/$\sqrt{\text{bar}}$ \\

$k_{\mathrm{ch}}$ 
& Choke-response steepness parameter controlling sensitivity to choke opening 
& -- \\

$\mathrm{ch}_{0}$ 
& Reference choke opening at which flow transitions from restricted to open 
& fraction \\

$C_{\mathrm{gl}}$ 
& Gas-lift utilization coefficient mapping injected lift gas to produced gas contribution 
& -- \\
\hline
\end{tabular}
\end{table}
