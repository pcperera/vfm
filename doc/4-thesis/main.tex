...
To support robust model development and unbiased evaluation, the available dataset is partitioned into training, validation, and test subsets on a per-well basis. 
The partitioning strategy is designed to (i) preserve temporal ordering within each subset, (ii) ensure representation of all wells across splits, and (iii) maintain sufficient sample sizes for model calibration and evaluation.
...
Overall, the hybrid model provides the most balanced and reliable performance for water rate prediction. While water remains the most challenging phase to model, the proposed framework significantly improves prediction accuracy compared to both the physics-based model and the MPFM baseline, demonstrating the effectiveness of physics-informed residual learning in capturing complex multiphase flow behavior.
...
\item \textbf{Hybrid model significantly improves predictive accuracy.}  
...
All models exhibit relatively lower and more variable performance for water rate prediction. Although the hybrid model significantly improves accuracy compared to the physics model, performance remains sensitive to sparse and intermittent water production patterns.
...
While some model configurations achieve slightly lower MAPE values, they do so at the cost of reduced $R^2$. In contrast, the hybrid model maintains competitive error levels while achieving significantly higher $R^2$, demonstrating a better balance between predictive accuracy and overall model fidelity.
...
\begin{itemize}
    \item Hybrid physics--ML modeling significantly improves production rate estimation under both temporal and spatial generalization, particularly for oil and gas rates.
