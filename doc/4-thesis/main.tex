...
The spatial generalization evaluation assesses the ability of the proposed modeling framework to transfer predictive capability to previously unseen wells. 
This evaluation is conducted using a LOWO validation strategy, which emulates realistic deployment conditions in which a trained model is applied to a new well with limited historical data.
In each LOWO experiment, one well is excluded from model development and used solely for evaluation, while the remaining wells constitute the training set. The machine-learning residual model is trained globally using data from the training wells and is kept fixed during evaluation on the unseen well. For the held-out well, limited calibration of well-specific physics parameters is performed using a small subset of available data to ensure physical consistency, while retraining of the residual model is intentionally avoided.

This setup enables isolation of the spatial transferability of the learned residual corrections across wells, while maintaining a realistic calibration procedure consistent with field operations. Both physics-only and hybrid model configurations are evaluated on the unseen well, and their performance is compared against MPFM estimates using well-test measurements as the reference standard.
Model performance is quantified using well-test measurements as the reference standard and evaluated using the evaluation metrics defined in Section~\ref{sec:evaluation_metrics} for oil, water, and gas flow rates. 
...
Based on the temporal generalization evaluation setup described in Section~\ref{sec:temporal_setup}, this section presents per-well prediction accuracy on unseen time periods.
...
Based on the LOWO evaluation protocol defined in Section~\ref{sec:lowo_setup}, this section reports model performance on previously unseen wells.
