...
Table~\ref{tab:temporal_qo} summarizes the per-well temporal generalization performance for oil rate prediction.

\begin{table}[!ht]
\centering
\caption{Temporal generalization performance for oil rate ($q_o$)}
\label{tab:temporal_qo}
\begin{tabular}{lcccc}
\hline
Well & $R^2$ & MAE & RMSE & MAPE (\%) \\
\hline
W06 & 0.98 & 3.85 & 4.93 & 3.84 \\
W08 & -15.28 & 35.56 & 44.08 & 255.01 \\
W10 & 0.26 & 17.56 & 28.89 & 18.93 \\
W11 & -0.82 & 59.59 & 74.09 & 13660.05 \\
W15 & -5.55 & 27.21 & 36.02 & 93.65 \\
W18 & -10.58 & 86.26 & 107.78 & 143.37 \\
W19 & -93.72 & 70.56 & 98.78 & 243.08 \\
\hline
\end{tabular}
\end{table}
...
Table~\ref{tab:temporal_qg} reports gas rate prediction performance under lag-safe temporal extrapolation.

\begin{table}[!ht]
\centering
\caption{Temporal generalization performance for gas rate ($q_g$)}
\label{tab:temporal_qg}
\begin{tabular}{lcccc}
\hline
Well & $R^2$ & MAE & RMSE & MAPE (\%) \\
\hline
W06 & 0.82 & 1686.47 & 1918.02 & 11.58 \\
W08 & -13.51 & 4517.19 & 5036.48 & 186.17 \\
W10 & -0.75 & 3269.41 & 4541.15 & 22.36 \\
W11 & -1.43 & 6569.36 & 8077.58 & 138.91 \\
W15 & -2.60 & 2362.72 & 3441.07 & 41.77 \\
W18 & -0.60 & 6951.38 & 8516.43 & 63.26 \\
W19 & -1.65 & 2677.68 & 3334.58 & 52.16 \\
\hline
\end{tabular}
\end{table}


\subsection{Water Rate and Ratio Metrics}

Table~\ref{tab:temporal_water} summarizes water rate ($q_w$), water--gas ratio (WGR), and gas--oil ratio (GOR) performance. These quantities are particularly sensitive to regime changes and measurement noise.

\begin{table}[!ht]
\centering
\caption{Temporal generalization performance for water-related quantities}
\label{tab:temporal_water}
\begin{tabular}{lcccc}
\hline
Quantity & Well & $R^2$ & MAE & RMSE \\
\hline
$q_w$ & W06 & -1.94 & 1.94 & 2.35 \\
      & W10 & -0.57 & 15.05 & 25.75 \\
      & W11 & -1.74 & 15.40 & 18.23 \\
\hline
WGR  & W06 & -1.60 & 0.00013 & 0.00015 \\
     & W10 & -1.23 & 0.00144 & 0.00274 \\
     & W11 & -0.68 & 0.00161 & 0.00257 \\
\hline
GOR  & W06 & -57.02 & 15.95 & 17.59 \\
     & W10 & -1.43 & 23.92 & 30.94 \\
     & W11 & -11.87 & 70.28 & 104.99 \\
\hline
\end{tabular}
\end{table}


\subsection{Global Temporal Performance}

To avoid statistical bias associated with averaging per-well metrics, global performance metrics were computed by pooling all valid samples across wells prior to evaluation. Table~\ref{tab:temporal_global} summarizes the global temporal generalization results.


\begin{table}[!ht]
\centering
\caption{Global temporal generalization performance (pooled across all wells)}
\label{tab:temporal_global}
\begin{tabular}{lcccc}
\hline
Quantity & Model & $R^2$ & MAE & RMSE \\
\hline
$q_o$ & Physics & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} \\
      & Hybrid  & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} \\
      & MPFM    & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} \\
\hline
$q_g$ & Physics & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} \\
      & Hybrid  & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} \\
      & MPFM    & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} \\
\hline
$q_w$ & Physics & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} \\
      & Hybrid  & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} \\
      & MPFM    & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} & \textit{(from GLOBAL)} \\
\hline
\end{tabular}
...
\subsection{Global Temporal Generalization Analysis}
Table~\ref{tab:temporal_global} reports global temporal generalization metrics computed by pooling all test samples across wells. 
In contrast to per-well evaluation, global metrics emphasize dominant production regimes and overall error mass.

The hybrid model demonstrates improved global performance over the physics-only baseline for oil and gas rates, confirming that residual learning captures systematic physics bias across wells. 
However, this improvement is not uniform across all assets, as evidenced by the per-well results. Water-related quantities remain challenging even under global evaluation, reflecting strong regime dependence and non-stationarity associated with water breakthrough.

These findings highlight the complementary nature of global and per-well evaluation: while global metrics reflect aggregate field-level performance, per-well analysis is essential for understanding model robustness and deployment risk.
